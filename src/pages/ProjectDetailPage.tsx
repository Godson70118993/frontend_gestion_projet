...
const ProjectDetailPage: React.FC = () => {
  ...
  const fetchProjectAndTasks = async () => {
    ...
    setLoading(true);
    try {
      const projectResponse = await fetch(`https://backend-gestion-projet-6.onrender.com/projects/${projectId}`, {
        headers: {
      ...
      const tasksResponse = await fetch(`https://backend-gestion-projet-6.onrender.com/projects/${projectId}/tasks`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      const response = await fetch(`https://backend-gestion-projet-6.onrender.com/tasks/${taskToDeleteId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
